...
! file names
CHARACTER (len=290)   :: input_filename
...
DO i=0,n_fields
  fileid=i+1
  WRITE(input_filename,"(A25,I2.2,A4)") "/input/irt_objects_input_",i,".srv"
  !input_filename = trim(odir)// trim("input/irt_objects_input_",i,".srv")
  write(*,*) trim(odir)//trim(input_filename) !input_filename
  write(*,*) i
  OPEN(fileid,FILE=trim(odir)//trim(input_filename),FORM='unformatted', ACTION='read')

ENDDO
!WRITE(*,*) input_filename(1)
output_filename    =trim(odir)// "/output//irt_objects_output.txt"
mask_filename      = trim(odir)// "/output//irt_objects_mask.srv"
coarsevel_filename = trim(odir)// "/output//irt_advection_field.srv"
...
!  write(*,*) trim(odir)// trim(input_filename(fileid))
!
...
ALLOCATE(field_mean(max_no_of_cells,2,n_fields+1))
ALLOCATE(field_min(max_no_of_cells,2,n_fields+1))
ALLOCATE(field_max(max_no_of_cells,2,n_fields+1))
