...
output_filename = trim(odir) //"/output/irt_advection_field.srv"
srv_header(1) = 1	    ! Code
...
DO
   ...
   nx = MIN(FLOOR((REAL(center_of_mass_x0)/REAL(dsize_x))*nx_bins)+1,nx_bins)
   ny = MIN(FLOOR((REAL(center_of_mass_y0)/REAL(dsize_y))*ny_bins)+1,ny_bins)
