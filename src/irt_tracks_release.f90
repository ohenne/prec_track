...
REAL, ALLOCATABLE    :: field_mean(:,:,:)
REAL, ALLOCATABLE    :: field_min(:,:,:)
REAL, ALLOCATABLE    :: field_max(:,:,:)
...
REAL, ALLOCATABLE    :: field_mean0(:),field_min0(:),field_max0(:)
REAL                 :: totarea0
...
CHARACTER (len=190)  :: input_filename  
CHARACTER (len=190)  :: output_filename
CHARACTER (len=190)  :: output2_filename 

namelist /INPUT2/ n_fields
namelist /INPUTIO/ odir 
open(1000,file='job/namelist.dat')
read(1000,nml=INPUT2)
read(1000,nml=INPUTIO)

ALLOCATE(field_mean(max_length_of_track,max_no_of_tracks,n_fields+1))
ALLOCATE(field_min(max_length_of_track,max_no_of_tracks,n_fields+1))
ALLOCATE(field_max(max_length_of_track,max_no_of_tracks,n_fields+1))
ALLOCATE(field_mean0(n_fields+1))
ALLOCATE(field_min0(n_fields+1))
ALLOCATE(field_max0(n_fields+1))
write(*,*) 'nfields', n_fields
input_filename  = trim(odir)//"/output//irt_objects_output.txt"
output_filename = trim(odir)//"/output//irt_tracks_output.txt"
output2_filename = trim(odir)//"/output//irt_tracks_nohead_output.txt"
...
ELSEIF (largest_forward_link .EQ. 0) THEN ! no forward link
  status_forward = 0
ELSEIF (second_largest_forward_link_size/largest_forward_link_size .LE. threshold_ratio) THEN 
  ! track is continued
  status_forward = 1
...
ELSEIF (largest_backward_link .EQ. 0) THEN ! no forward link
  status_backward = 0
ELSEIF (second_largest_backward_link_size/largest_backward_link_size .LE. threshold_ratio) THEN 
  ! track is continued
  status_backward = 1
...
! If cell_number0 continuates a track, it could terminate smaller ones, dependent on threshold_ratio
